...
declare -r version='SPARK JOB  v1.1.0'
...
declare SPARKJOB_DELAY_BASE=15
declare SPARKJOB_DELAY_MULT=0.125
...
mysubmit() {
	...
	local -a opt=(
		...
		--env "SPARKJOB_DELAY_BASE=$SPARKJOB_DELAY_BASE"
		--env "SPARKJOB_DELAY_MULT=$SPARKJOB_DELAY_MULT"
...
if ((interactive>0));then
	...
	if [[ -s $SPARKJOB_WORKING_ENVS ]];then
		...
		declare -r runbash="exec bash --rcfile <(
			...
			echo SPARKJOB_DELAY_BASE=\'$SPARKJOB_DELAY_BASE\';
			echo SPARKJOB_DELAY_MULT=\'$SPARKJOB_DELAY_MULT\';
			echo source ~/.bashrc;
